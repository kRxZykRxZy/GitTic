...
export const UserManagement: React.FC = () => {
    ...
    const {
        ...
        async (pg, pp) => {
            const apiResp = await adminService.getUsers({
                page: pg,
                perPage: pp,
                search: searchTerm || undefined,
            });
            if (apiResp.success && apiResp.data && apiResp.data.pagination) {
                return {
                    success: true,
                    data: {
                        items: apiResp.data.data as User[],
                        data: apiResp.data.data as User[],
                        page: apiResp.data.pagination.page,
                        perPage: apiResp.data.pagination.perPage,
                        total: apiResp.data.pagination.total,
                        totalPages: apiResp.data.pagination.totalPages,
                        hasNext: apiResp.data.pagination.page < apiResp.data.pagination.totalPages,
                        hasPrev: apiResp.data.pagination.page > 1,
                        pagination: apiResp.data.pagination,
                    },
                    timestamp: apiResp.timestamp || new Date().toISOString(),
                };
            }
            // Return a default empty PaginatedResponse<User> for error case
            return {
                success: false,
                message: apiResp.message || 'Failed to fetch users',
