 ...
 */
export function toMilliseconds(duration: Partial<Duration>): number {
  const {
    days = 0,
    hours = 0,
    minutes = 0,
...
export function formatDuration(ms: number, verbose: boolean = false): string {
  ...
  if (d.minutes > 0) {
    parts.push(
      verbose ? `${d.minutes} minute${d.minutes !== 1 ? "s" : ""}` : `${d.minutes}m`,
    );
  }
  if (d.seconds > 0) {
    parts.push(
      verbose ? `${d.seconds} second${d.seconds !== 1 ? "s" : ""}` : `${d.seconds}s`,
    );
  }
  if (parts.length === 0) {
    parts.push(
 ...
 * ```
 */
export function parseShortDuration(input: string): number {
